...
type gameStatus = {
    pop: { frame: number, px: number, py: number, value: string, color?: string, boldColor?: number }[]
    lasers: { x: number, y: number, frame: number }[]
    stereo: { [key: string]: { source: BufferSource, L: GainNode, R: GainNode } }
    absorb: { frame: number, ctx: CanvasRenderingContext2D }[]
...
declare class plugin {
     ...
     * @param name 音效名
     * @param left 左声道音量，默认为1
     * @param right 右声道音量，默认为1
     * @param split 音效为双声道，请填true，为单声道，请填false或不填
     */
    playStereo(name: string, left: number, right: number, split?: boolean): number

    /** 移动声源（渐变调整左右声道音量）
     * @param id 为playStereo返回的id
     * @param left 要渐变到的左声道音量
     * @param right 要渐变到的右声道音量
     * @param time 渐变时间
     */
    moveStereo(id: number, left: number, right: number, time: number): void
}
...
declare var flags: { [key: any]: any; skillLevel: { [key: string]: number } }