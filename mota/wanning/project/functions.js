var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = {
	"events": {
		...
		"lose": function (reason) {
			// 游戏失败事件
			core.ui.closePanel(reason === '被自爆阵炸死' ? true : false);
			var replaying = core.isReplaying();
			core.stopReplay();
			core.waitHeroToStop(function () {
				core.drawText([
		...
		"afterChangeFloor": function (floorId) {
			...
			// 自動咸綠 第九大陆抄来的
			if (core.getFlag('autoGreen', 0) === true) {
				(function () {
					var actions = core.searchBlock("greenDoor").map(function (block) {
						return { "type": "openDoor", "loc": [block.x, block.y], "needKey": true, "async": true };
					});
					if (actions.length > 0) {
			...
			if (core.hasFlag('__fromLoad__')) {
				...
				// 每次抵达楼层执行的事件
				var todo = core.floors[floorId].eachArrive;
				core.insertAction(todo);

				// 首次抵达楼层时执行的事件（后插入，先执行）
				if (!core.hasVisitedFloor(floorId)) {
					var todo = core.floors[floorId].firstArrive;
					todo.push({ 'type': 'function', 'function': 'function(){core.openUpgrade(\'' + floorId + '\')}' });
					core.insertAction(todo);
		...
		"afterBattle": function (enemyId, x, y, option) {
			...
			if (!option.explode)
				core.addPop(x * 32 + 16, y * 32 + 16, core.formatBigNumber(core.status.thisMap.hps[x + ',' + y]));

			// 获得金币
			core.status.hero.money += enemy.money;
			core.status.hero.statistics.money += enemy.money;

			// 获得经验
			core.status.hero.exp += enemy.exp;
			core.status.hero.statistics.exp += enemy.exp;

			// 执行勇士的技能
			if (!option.explode) {
				core.explode(x, y);
				core.laser(x, y);
				core.chainHero(enemyId, x, y);
				core.chainBoom(enemyId, x, y);
			}

			// 执行阵法
			if (!special.some(function (v) { return v >= 28 && v <= 100 })) {
				core.doFormation(x, y, core.status.floorId, option.eExplode || 0);
			}

			// 添加杀敌数
			core.addKilled(1, core.status.floorId);
			...
			// 轮回阵
			if (core.status.thisMap.formation.includes(32) && !enemy.notBomb) {
				var level = core.status.thisMap.formationLevel[core.status.thisMap.formation.indexOf(32)];
				var relive = core.status.thisMap.relive;
				if (!relive) relive = core.status.thisMap.relive = {};
				if (!relive[x + ',' + y]) relive[x + ',' + y] = 1;
				else relive[x + ',' + y]++;
				if (relive[x + ',' + y] <= level) core.setBlock(enemyId, x, y);
			}

			core.saveHp(core.status.floorId);
			...
			if (!option.explode && core.status.hero.hp <= 0) {
				core.lose();
			}
		},
	...
	"enemys": {
		"getSpecials": function () {
			...
			return [
				...
				[28,
					function (enemy) { var s = Math.min(enemy.square || 1, 12); return s + '级四方阵' },
					function (enemy) {
						var s = Math.min(enemy.square || 1, 12);
						var l = s * 2 + 1;
						return '一个基础但十分强劲的阵法，该阵法在场时，地图上所有的怪物死亡后会在周围' + l + '×' + l + '范围内重新生成被打败怪物，不覆盖原有图块';
					},
					'#7799ee'
				],
				[29,
					function (enemy) { return (enemy.strengthen || 1) + '级强化阵' },
					function (enemy) {
						return '一个十分强劲的阵法，该阵法在场时，所有的阵法都会升级' + (enemy.strengthen || 1) + '级';
					},
					'rgb(255, 215, 0)'
				],
				[30,
					function (enemy) { return (enemy.chain || 1) + '级连锁阵' },
					function (enemy) {
						var s = Math.min(enemy.chain || 1, 12);
						var l = s * 2 + 1;
						return '一个十分强劲的阵法，该阵法在场时，勇士打败一个怪物后，所有同种怪物周围' + l + '×' + l + '范围内重新生成被打败怪物，不覆盖原有图块';
					},
					'rgb(255, 0, 255)'
				],
				[31,
					function (enemy) { return (enemy.explode || 1) + '级自爆阵' },
					function (enemy) {
						var l = enemy.explode * 2 + 1;
						l = core.clamp(l, 0, 7);
						return '一个几乎无敌的阵法，该阵法在场时，怪物死亡时，会产生一个' + l + '×' + l + '的爆炸，对范围内的所有己方和敌方造成 自身初始生命值×' + ((enemy.explode || 0) * 10) + '% 的可被魔防抵消的伤害';
					},
					'rgb(255, 0, 0)'
				],
				[32,
					function (enemy) { return (enemy.relive || 1) + '级轮回阵' },
					function (enemy) {
						return '一个十分强劲的阵法，该阵法在场时，每个怪物都可以复活' + (enemy.relive || 1) + '次';
					},
					'#81FF5E'
				],
				[33,
					function (enemy) { return (enemy.absorb || 1) + '级吸收阵' },
					function (enemy) {
						var l = enemy.absorb * 2 + 1;
						l = core.clamp(l, 0, 7);
						return '一个几乎无敌的阵法，该阵法在场时，所有拥有阵法技能的怪物在勇士每次攻击怪物后会吸收周围' + l + '×' + l + '范围内的所有怪物的生命值的' + (enemy.absorb * 50) + '%，转化为自己的生命';
					},
					'#00FFD5'
				],
			];
	...
	"control": {
		"saveData": function () {
			...
			var data = {
				...
				'hps': core.clone(core.saveHpsToSave()), // 保存hp数据
				'killed': core.clone(core.saveKilled()) // 保存杀敌数
			};
		...
		"loadData": function (data, callback) {
			...
			// 将怪物血量读取出来 和 杀敌数
			for (var floor in data.hps) {
				core.status.maps[floor].hps = core.clone(data.hps[floor]);
				core.status.maps[floor].killed = data.killed[floor]
			}
		...
		"updateStatusBar": function () {
			...
			if (core.domStyle.isVertical) core.dom.hard.style.fontSize = '1.4em';
			else core.dom.hard.style.fontSize = '2em';

			core.setStatusBarInnerHTML('openFormation', '查看阵法', 'color: #0ff');
			core.setStatusBarInnerHTML('openSkill', '查看技能', 'color: #f0f');
		...
		"moveOneStep": function (callback) {
			...
			// 播放足音 立体声
			core.playStereo('Step0.mp3', 1 - nowx / 12, nowx / 12, false);
